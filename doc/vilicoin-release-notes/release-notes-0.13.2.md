...
ECDSA signatures inside Vilicoin transactions now use validation using
...
Previous versions of Vilicoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Vilicoin Core was to raise the default minimum
relay fee.

Vilicoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Vilicoin Core 0.13.2 also introduces new default policy limits on the length and
...
Vilicoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Vilicoin Core will relay transactions with
...
entirely in Vilicoin Core 0.13, and it is at this time undecided whether the
...
Vilicoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Vilicoin Core automatically creates a hidden service to listen on, without
manual configuration. Vilicoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Vilicoin Core is listening, and
...
Vilicoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Vilicoin
Core to use floating fees. By default, Vilicoin Core will use floating
fees.
...
(default: `0.0002` VILI/kB).

At all times, Vilicoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) VILI.
Furthermore, Vilicoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Vilicoin Core code base has
...
for a new opcode in the Vilicoin scripting system that in combination with
...
Vilicoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Vilicoin Core.
...
The two phases are distinct in the Vilicoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Vilicoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Vilicoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Vilicoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Vilicoin addresses (both P2PKH addresses
  ...
  believed to be provided by Vilicoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Vilicoin's current ledger (called the UTXO set),
  ...
  note that Vilicoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Vilicoin users to individually opt-in to almost any change in the Vilicoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Vilicoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Vilicoin
client to Vilicoin Core 0.13.2 to signal for segwit support. When a
...
Since Vilicoin Core 0.10.0, nodes have defaulted to only relaying and
